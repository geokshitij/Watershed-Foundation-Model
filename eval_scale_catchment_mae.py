...
def create_data_splits(root_dir, train_split=0.8, val_split=0.1, seed=42):
    """
    Lists all TIFF files, shuffles them, and splits them into train, val, and test sets.
    Saves the filenames to .txt files.
    """
    ...
    num_test = int(num_files * (1.0 - train_split - val_split)) # The rest is test
    ...
    for name, file_list in [("train", train_files), ("val", val_files), ("test", test_files)]:
        path = os.path.join(root_dir, f"{name}.txt")
...
class TiffCatchmentDataset(Dataset):
    ...
    It now accepts a list of filenames to use for a specific data split (train/val/test).
    """
    def __init__(self, root_dir, file_list, transform=None, nodata_value=-9999.0, image_size=224):
        ...
        self.image_files = file_list # Use the provided list of files

        if not self.image_files:
            raise RuntimeError(f"Dataset initialized with an empty file list for directory {root_dir}")
...
def run_training_thread(config):
    ...
    try:
        with redirect_stdout(training_log_capture):
            ...
            if len(train_dataset) == 0: raise RuntimeError("Training set is empty. Check your input directory and file extensions.")
...
def index():
    # The HTML template remains the same as it doesn't need to know about the splits
    html_template = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
