...
try:
    ...
    print("pip install lightly timm flask tifffile numpy imagecodecs torchvision matplotlib")
    sys.exit(1)
...
def run_training_thread(config):
    ...
    try:
        with redirect_stdout(training_log_capture):
            ...
            if not training_status["stopped"]:
...
def embed():
    ...
    # Infer config path from model path, assuming it's in the same directory
